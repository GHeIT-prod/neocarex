...
class SpherePayment
{
    ...
    /**
     * Constructor
     * @param SessionInterface|null
     */
    public function __construct(private readonly string $front, private readonly int $patientIdCc, private readonly ?SessionInterface $session = null)
    {
        ...
        $this->mainUrl = $url . '?aggregators=' . urlencode(Sphere::AGGREGATOR_ID) . '&trxcustid=' . urlencode($trxcustid) . '&trxcustid_licensekey=' . urlencode($trxcustidLicensekey) . '&trxcustomfield[1]=' . urlencode($frontSpecific) . '&trxcustomfield[2]=' . urlencode($this->patientIdCc) . '&trxcustomfield[3]=' . urlencode((string) CsrfUtils::collectCsrfToken('sphere', $this->session));
        if ($this->front == 'clinic') {
            $this->mainUrlRetail = $url . '?aggregators=' . urlencode(Sphere::AGGREGATOR_ID) . '&trxcustid=' . urlencode($trxcustidRetail) . '&trxcustid_licensekey=' . urlencode($trxcustidRetailLicensekey) . '&trxcustomfield[1]=' . urlencode($frontSpecificRetail) . '&trxcustomfield[2]=' . urlencode($this->patientIdCc) . '&trxcustomfield[3]=' . urlencode((string) CsrfUtils::collectCsrfToken('sphere', $this->session));
        }
    ...
    private function renderSphereJsCore(): string
    {
        return "
            ...
            function paySphere(front) {
                ...
                let cancelUrl = " . js_escape($this->serverSite) . " + '/sphere/initial_response.php?cancel=cancel&ticket=' + encodeURIComponent(ticket) + '&front=' + encodeURIComponent(front) + '&patient_id_cc=' + " . js_escape($this->patientIdCc) . " + '&csrf_token=' + " . js_escape(CsrfUtils::collectCsrfToken('sphere', $this->session)) .  ";
